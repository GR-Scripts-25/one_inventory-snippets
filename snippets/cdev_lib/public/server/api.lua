...
clib_API.Inventory = {
    HasItem = function(source, item, amount)
        ...
        elseif clib_Config.public.Inventory == "one" then
            return exports.one_inventory:HasItem(source, item, amount or 1)
        else
            return false
        end
    end,
        ...
        if clib_Config.public.Inventory == "qbcore" then
            ...
            for _, item in pairs(gloveboxItems) do
                if string.lower(item.name) == string.lower(itemName) then
                    return tonumber(item.amount) >= (amount or 1)
                end
            end
            return false
        elseif clib_Config.public.Inventory == "ox" then
            local items = exports["ox_inventory"]:GetInventoryItems("glovebox" .. plate)
            for _, item in pairs(items or {}) do
                if string.lower(item.name) == string.lower(itemName) then
                    return tonumber(item.count) >= (amount or 1)
                end
            end
            return false
        elseif clib_Config.public.Inventory == "one" then
            return exports.one_inventory:HasItem("glovebox:" .. plate, itemName, amount or 1)
        else
            return false
        end
    end,
    ...
    HasItemInTrunk = function(plate, itemName, amount)
        ...
        elseif clib_Config.public.Inventory == "ox" then
            ...
            return false
        elseif clib_Config.public.Inventory == "one" then
            return exports.one_inventory:HasItem("trunk:" .. plate, itemName, amount or 1)
        else
            return false
        end
    end,
    
    RemoveItem = function(source, item, amount, slot, metadata)
        if clib_Config.public.Inventory == "qbcore" then
        ...
        elseif clib_Config.public.Inventory == "one" then
            return exports.one_inventory:RemoveItem(source, item, amount or 1, metadata, slot)
        end
    end,
    
    AddItem = function(source, item, amount, slot, metadata)
        if clib_Config.public.Inventory == "qbcore" then
            return exports["qb-inventory"]:AddItem(source, item, amount, slot, metadata)
        elseif clib_Config.public.Inventory == "ox" then
        ...
        elseif clib_Config.public.Inventory == "one" then
            return exports.one_inventory:AddItem(source, item, amount or 1, metadata, slot)
        end
    end,
    
    RegisterStash = function(id, label, slots, maxWeight, owner)
        if clib_Config.public.Inventory == "ox" then
            exports["ox_inventory"]:RegisterStash(tostring(id), label, slots, maxWeight, owner)
        elseif clib_Config.public.Inventory == "one" then
            print("^2[clib] one_inventory: Stash '" .. id .. "' is ready (auto-create on first use)^7")
        end
    end,
    ...
    OpenStash = function(source, data)
        ...
        elseif clib_Config.public.Inventory == "one" then
            if type(data) == "string" then
                exports.one_inventory:OpenInventory(source, 'stash', { id = data })
            else
                exports.one_inventory:OpenInventory(source, 'stash', data)
            end
        end
    end
}
