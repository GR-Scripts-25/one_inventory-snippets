if not Config.LBPhone or Config.Item.Inventory ~= "one_inventory" then
    return
end
...
local function GetPhonesInInventory(source)
    ...
    if phoneConfig.Item.Name then
        return exports.one_inventory:SearchInventory(source, phoneConfig.Item.Name) or {}
    end

    local phones = {}

    for i = 1, #phoneConfig.Item.Names do
        local items = exports.one_inventory:SearchInventory(source, phoneConfig.Item.Names[i].name) or {}
