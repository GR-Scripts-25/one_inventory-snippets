...
function CanCarryItem(xPlayer, item, amount)
    ...
    if Config.Inventory == 'one' then
        return exports.one_inventory:CanCarryItem(GetSource(xPlayer), item, amount)
    end
...
function AddItem(xPlayer, item, amount, metadata)
    ...
    if Config.Inventory == 'one' then
        exports.one_inventory:AddItem(GetSource(xPlayer), item, amount, metadata)
        return true
    end
...
function RemoveItem(xPlayer, item, amount)
    ...
    if Config.Inventory == 'default' and IsWeapon(item) then
        exports.one_inventory:RemoveItem(GetSource(xPlayer), item, amount)
        return
    end
...
function GetItemLabel(item)
    ...
    if Config.Inventory == 'one' then
        return exports.one_inventory:CreateItemsDefinition(item)?.label or item
    end
